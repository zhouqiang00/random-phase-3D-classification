...
// Modified by ZhouQ
void MlOptimiserMpi::randomize_phase_otherrefs()
{
	if (verb > 0)
	{
		std::cout << "Replace all refs other than first one with phase-randomized first reference." << std::endl;
	}
	...
	// only randomize corresponding classes
	for (int iclass = 1; iclass < mymodel.nr_classes; iclass++)
	{
		int random_phase_rank = (iclass % (node->size - 1) ) + 1;
		if (node->rank == random_phase_rank)
		{
...
void MlOptimiserMpi::get_phase_randomized_refs()
{
	if (verb > 0)
	{
		std::cout << "Broadcast phase-randomized references." << std::endl;
	}
	// Broadcast the phase-randomized refs to all other MPI nodes
	for (int iclass = 1; iclass < mymodel.nr_classes; iclass++)
	{
		...
		node->relion_MPI_Bcast(MULTIDIM_ARRAY(mymodel.Iref[iclass]),
				MULTIDIM_SIZE(mymodel.Iref[iclass]), MPI_DOUBLE, random_phase_rank, MPI_COMM_WORLD);
	}
	// Re-set the origin 
...
void MlOptimiserMpi::initialise()
{
	...
	// Modified by ZhouQ
 	// Phase-randomize and broadcast all references than first one
	if (do_random_phase_otherrefs)
	{
		randomize_phase_otherrefs();
		MPI_Barrier(MPI_COMM_WORLD);
		get_phase_randomized_refs();
	}
	MPI_Barrier(MPI_COMM_WORLD);
