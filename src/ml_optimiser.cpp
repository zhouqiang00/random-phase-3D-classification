...
{
	std::cout <<  "Modified by ZhouQ at 2016/03/21" << std::endl;
...
void MlOptimiser::parseInitial(int argc, char **argv)
{
	...
	// Modified by ZhouQ
	do_random_phase_otherrefs = parser.checkOption("--random_phase_otherrefs","Randomize phase of other references than first one.");
	random_phase_res = textToFloat(parser.getOption("--random_phase_res", "Resolution beyond which other references will be randomized", "100."));
	random_phase_res_upper = textToFloat(parser.getOption("--random_phase_res_upper_limit", "Upper limit of resolution at which other references will be randomized", "1."));
 	// Check conflict between random_phase_ref and nr_classes
	if (do_random_phase_otherrefs && mymodel.nr_classes < 2 )
	{
		if (verb > 0)
		{
			std::cerr << "The number of classes should be larger than 1 if randomize phase of other references. exiting..." << std::endl ;
		}
		exit(1);
	}
...
void MlOptimiser::initialise()
{
	...
	// Modified by ZhouQ
	if (do_random_phase_otherrefs)
	{
		int random_phase_index;
		random_phase_index = ROUND(mymodel.ori_size * mymodel.pixel_size / random_phase_res);
		int random_phase_index_upper;
		random_phase_index_upper = ROUND(mymodel.ori_size * mymodel.pixel_size / random_phase_res_upper);
		for (int iclass = 1; iclass < mymodel.nr_classes; iclass++)
		{
			...
			randomizePhasesBeyond(mymodel.Iref[iclass], random_phase_index, random_phase_index_upper);
		}
	}
